...
#define _GNU_SOURCE  /* for getline(3), from stdio.h */
...
// used in mpc_player.cpp
mpd_Connection * gconn;

// defined in mpc_player.cpp
extern void mpc_p4sync_startup();
extern void mpc_p4sync_shutdown();
...
mpd_Connection * setup_connection ()
{
	...
	gconn = mpd_newConnection(host+parsed_len,iport,10);

	if(gconn->error && (!port_env || !host_env)) 
		fprintf(stderr,"MPD_HOST and/or MPD_PORT environment variables"
			" are not set\n");
	
	printErrorAndExit(gconn);

	if(password_len) 
		send_password (host, password_len, gconn);
...
/* tokenize a command line, using whitespace as the delimiter. (the whitespace
   is discarded.) returns the number of tokens. */
int tokenize(char *line, char *command[]) {
	int i = 0;
...
int main(int argc, char ** argv)
{
	...
	assert(line);
	printf("Enter commands, one per line. End with ^D or an empty line.\n");

	mpc_p4sync_startup();

	while (getline(&line, &line_len, stdin) > 0) {
		num_commands = tokenize(line, command);
		if (num_commands == 0)
			break;

		for (i = 0; mpc_table[i].command; ++i) {
			if ( ! strcmp(command[0], mpc_table[i].command) ) {
				mpd_Connection * conn = setup_connection();
				char ** array = check_args (i, &num_commands, command);
				if (!array)
					continue;

				/* not a typo, assignment intended */
				if( (ret=mpc_table[i].handler(num_commands, array, conn)))
					print_status(conn);
			
				if (0>mpc_table[i].pipe)
					free_pipe_array(num_commands,array);
